...
class FedProxSGD(Optimizer):
    ...
    def __init__(self, params, lr=required, momentum=0, dampening=0,
        ...
        self.comm_thread = threading.Thread(
                target=FedProxSGD._async_all_reduce_,
    ...
    def step(self, anchor_model, mu=0, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p, delta in zip(group['params'], anchor_model.parameters()):
                if p.grad is None:
                    continue
                d_p = p.grad.data
                # if 'anchor_model' not in param_state:
                #     buf = param_state['anchor_model'] = torch.clone(p.data).detach()
                #     self.comm_buf.append(buf)
                datashape = d_p.shape
                ...
                # if 'variance' not in param_state:
                #     var = param_state['variance'] = torch.zeros(datashape).to(device)
                # else:
                #     var = param_state['variance']


                # d_p.sub_(var)
                # print(d_p.shape, delta.shape)
                d_p.add_(mu, p.data - delta.data)
    ...
    def async_BMUF_multithreads(self, itr, cp, req):
        # Olocal SGD
        step_flag = (itr != 0 and itr % cp == 0)
        if step_flag:
    ...
    def async_VRO_SGD_step(self, itr, cp, req):
        step_flag = (itr != 0 and itr % cp == 0)
        if step_flag:
            ...
            for group in self.param_groups:
                ...
                for p in group['params']:
                    ...
                    var = param_state['variance']
                    # var.add_(1 / (cp*lr), (p.data - old_data))
                    #param_state['momentum_buffer'].zero_()
    ...
    def update_alpha(self, epoch):
        if epoch in self.alpha_schedule:
