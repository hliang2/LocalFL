...
class DataPartitioner(object):
    ...
    def __init__(self, data, sizes=[0.7, 0.2, 0.1], seed=1234, isNonIID=False, alpha=0):
        ...
        if isNonIID:
            ...
            self.partitions = self.__getNonIIDdata__(data, sizes, seed, alpha)
            self.ratio = sizes
    ...
    def __getDirichletData__(self, data, psizes, seed, alpha):
        ...
        rng = Random()
        rng.seed(seed)
        a = [(label.item(), idx) for idx, label in enumerate(labelList)]
        # Same Part
        labelIdxDict = dict()
        for label, idx in a:
            labelIdxDict.setdefault(label,[])
...
def partition_dataset(rank, size, args):
    ...
    if args.dataset == 'emnist':
        ...
        train_dataset = torchvision.datasets.EMNIST(root='/users/jianyuw1/AdaDSGD/data',
                                              ...
                                              transform=transform_train) 
        partition_sizes = [1.0 / size for _ in range(size)]
        partition = DataPartitioner(train_dataset, partition_sizes, isNonIID=args.NIID, alpha=args.alpha)
        ratio = partition.ratio
        partition = partition.use(rank)
...
def select_model(num_class, args):
    if args.model == 'VGG':
        ...
        if args.dataset == 'cifar10':
            ...
            # model = ResNet18()
            model = resnet_small.ResNet(50, 10)
        elif args.dataset == 'imagenet':
        ...
        model = logistic.LogisticRegression(60, 10)
